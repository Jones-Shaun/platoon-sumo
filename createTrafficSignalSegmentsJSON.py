...
includes traffic signal control using TraCI.
...
This version also saves the mapping between traffic signal indices and
the incoming lanes/edges they control to a JSON file during initialization.
This file can be used to help populate the MAIN_ROAD_GREEN_PHASES dictionary.
"""
import sys
import os
...
# Define total simulation time in seconds (and steps, as one step is 1 second)
SIM_TIME = 3600
...
# Use the output printed by this script during initialization or the generated JSON file
# to help you populate this dictionary.
...
MAIN_ROAD_GREEN_PHASES = {
    ...
    # Add all your traffic lights and their main road green phase indices here:
}
...
def run_simulation():
    ...
    # --- Generate and Save Traffic Signal to Incoming Lane Mapping ---
    traffic_signal_mapping = {} # Dictionary to store the mapping
    if not traffic_light_ids:
        print("No traffic lights found in the loaded network. No mapping file will be generated.")
    else:
        print("\n--- Generating Traffic Light Signal to Incoming Lane Mapping ---")
        for tl_id in traffic_light_ids:
            traffic_signal_mapping[tl_id] = {} # Create an entry for each traffic light

            # Get the controlled links for this traffic light
            controlled_links = traci.trafficlight.getControlledLinks(tl_id)

            # Flatten the list of lists to get a single list of all controlled links in order
            flat_controlled_links = [link for sublist in controlled_links for link in sublist]

            # Iterate through the links and store the incoming lane and edge for each signal
            for i, link_tuple in enumerate(flat_controlled_links):
                incoming_lane = link_tuple[0] # The first element of the tuple is the incoming lane ID

                # Get the edge ID the lane belongs to
                try:
                    edge_id = traci.lane.getEdgeID(incoming_lane)
                except traci.TraCIException:
                     edge_id = "N/A (Lane not found)" # Handle cases where lane might not be valid

                # Store the mapping for this signal index
                traffic_signal_mapping[tl_id][i] = {
                    "incoming_lane": incoming_lane,
                    "edge_id": edge_id
                }
                # Also print for immediate reference
                print(f"  Traffic Light '{tl_id}', Signal {i}: Controls traffic from Lane '{incoming_lane}' (Edge: '{edge_id}')")

        # Save the mapping to a JSON file
        try:
            with open(TRAFFIC_SIGNAL_MAPPING_FILE, 'w') as f:
                json.dump(traffic_signal_mapping, f, indent=4) # Use indent for readability
            print(f"\nTraffic signal mapping saved to '{TRAFFIC_SIGNAL_MAPPING_FILE}'")
            print("Use this file to help populate the MAIN_ROAD_GREEN_PHASES dictionary.")
        except IOError as e:
            print(f"Error saving traffic signal mapping to JSON file: {e}")

        print("--------------------------------------------------")
    # --- End Generate and Save Mapping ---
    ...
    for tl_id in traffic_light_ids:
        ...
        if tl_id not in MAIN_ROAD_GREEN_PHASES:
             ...
             print(f"Warning: Traffic light '{tl_id}' not found in MAIN_ROAD_GREEN_PHASES. Please define its main road green phase indices.")
                ...
                for link_set in controlled_links:
                    for link in link_set:
                        ...
                        if incoming_lane:  # Ensure incoming lane exists
                             # You might need to add logic here to check if this
                             # incoming_lane is part of a main‑road approach for
                             # this traffic light.
                             edge_id_of_lane = traci.lane.getEdgeID(incoming_lane)

                             # Check if the lane's edge is on one of the defined
                             # main‑road approaches
                             is_on_main_road_approach = (
                                    edge_id_of_lane in northbound_edges
                                    or edge_id_of_lane in southbound_edges
                             )  # Example check

                             if is_on_main_road_approach:
                                vehicles_on_lane = traci.lane.getLastStepVehicleIDs(incoming_lane)

                                # Corrected indentation for the vehicle loop
                                for veh_id in vehicles_on_lane:
                                    ...
                                    try:
                                        ...
                                        # platoon_id = simpla.getPlatoonID(veh_id)
                                        # if platoon_id != -1:
                                        #     is_in_platoon = True
                                        # Fallback: Check if 'platoon' is in the vehicle type name
                                        if 'platoon' in veh_type.lower():
                                            is_in_platoon = True
                                    except AttributeError:
                                        # If simpla.getPlatoonID doesn't exist, rely on vType check
                                        if 'platoon' in veh_type.lower():
                                            is_in_platoon = True
